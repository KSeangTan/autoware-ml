...
def multi_task_eval_output(
    multi_task_predictions: MultiTaskPredictions, multi_task_features: MultiTaskFeatures
) -> dict[str, Any]:
    """
    Pair decoded predictions with ground truth for the detection metric.
    ...
    Args:
        ...
        multi_task_features: MultiTaskFeatures containing the ground-truth boxes and labels.
    ...
    if multi_task_features.multi_task_gt_batch.detection3d_gt_batch is None:
        raise ValueError(
            "MultiTaskFeatures must contain detection3d_gt_batch for multi_task_eval_output."
        )

    return detection_eval_output(
        predictions=multi_task_predictions.to_list(),
        batch={
            "gt_boxes": multi_task_features.multi_task_gt_batch.detection3d_gt_batch.gt_bboxes_3d,
            "gt_labels": multi_task_features.multi_task_gt_batch.detection3d_gt_batch.gt_labels_3d,
            "gt_num_points": multi_task_features.multi_task_gt_batch.detection3d_gt_batch.gt_bboxes_num_points,
        },
    )
