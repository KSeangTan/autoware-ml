...
class PointPillarsScatter(nn.Module):
    ...
    def __init__(self, in_channels: int, output_shape: Sequence[int], batch_size: int) -> None:
        """Initialize the dense BEV scatter module.

        Args:
            in_channels: Input feature dimension.
            output_shape: Output BEV shape as ``(height, width)``.
            batch_size: Batch size of the current experiment.
        """
        super().__init__()
    ...
    def forward(
        ...
        Returns:
            Dense BEV feature map after scattering pillar features into their respective
        ...
        canvas = pillar_features.new_zeros((self.batch_size * height * width, self.in_channels))
        ...
        return (
            canvas.view(self.batch_size, height, width, self.in_channels)
            .permute(0, 3, 1, 2)
            .contiguous()
        )
