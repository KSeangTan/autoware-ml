...
from autoware_ml.models.multi_task_base import LogDictConfigs
...
class TestCenterPointDetectionModel(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.num_classes = 3
        torch.manual_seed(0)
        self.data_preprocessor = MultiTaskDataPreprocessor(
    ...
    def _build_multi_task_features(self) -> MultiTaskFeatures:
        """
        Build a MultiTaskFeatures batch with voxelized lidar inputs and detection ground truth
    ...
    def test_centerpoint_forward_compute_metrics_and_decode_run(self) -> None:
        ...
        """
        multi_task_features = self._build_multi_task_features()

        multi_task_outputs = self.centerpoint(multi_task_features)
        metrics = self.centerpoint.compute_metrics(multi_task_features, multi_task_outputs)
        multi_task_predictions = self.centerpoint.decode_outputs(multi_task_outputs)

        self.assertIsNotNone(multi_task_outputs.detection3d_outputs)
        assert multi_task_outputs.detection3d_outputs is not None
        center_head_outputs = multi_task_outputs.detection3d_outputs.center_head_outputs
