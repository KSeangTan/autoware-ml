...
def load_matching_weights(
    ...
    *,
    ...
    Only tensors with the same state-dict key and shape are loaded. Checkpoint
    tensors the model has no key for are reported and skipped. A tensor whose key
    matches but whose shape differs is an error unless ``skip_mismatched_shapes``
    is set, in which case it is skipped with a warning and the model keeps its own
    initialization for that tensor. This is intended for model initialization from
    pretrained weights, not for training resume.

    Args:
        model: Target model instance.
        checkpoint_path: Lightning checkpoint path.
        map_location: Device or device string used when loading the checkpoint.
        skip_mismatched_shapes: Skip, instead of rejecting, checkpoint tensors whose
            key matches a model tensor of a different shape.
    ...
    Raises:
        ValueError: If a checkpoint tensor matches a model key with a different shape
            and ``skip_mismatched_shapes`` is false, or if no checkpoint tensors match
            the target model.
    """
    ...
    if shape_mismatched_keys:
        ...
        active_logger.warning(
            "%s. Skipping them, the model keeps its own initialization for these tensors.",
            mismatch_message,
        )

    # load_state_dict(strict=False) tolerates missing and unexpected keys and reports
    # them, but it raises on a shape mismatch, so those tensors are the only ones
    # dropped up front.
    mismatched = frozenset(shape_mismatched_keys)
    candidate_state_dict = {
        key: value for key, value in checkpoint_state_dict.items() if key not in mismatched
    }
    incompatible_keys = model.load_state_dict(candidate_state_dict, strict=False)
    unexpected_keys = tuple(sorted(incompatible_keys.unexpected_keys))
    not_loaded_model_keys = tuple(sorted(incompatible_keys.missing_keys))
    loaded_keys = tuple(sorted(set(candidate_state_dict) - set(unexpected_keys)))
    if not loaded_keys:
        raise ValueError(
    ...
    active_logger.info(
        "Skipped checkpoint keys with mismatched shapes (%d): %s",
        len(shape_mismatched_keys),
        _format_keys(shape_mismatched_keys),
    )
    active_logger.info(
...
def apply_matching_weights(
    ...
    *,
    ...
    Args:
        ...
        skip_mismatched_shapes: Skip, with a warning, checkpoint tensors whose key
            matches a model tensor of a different shape instead of raising. The model
            keeps its own initialization for them, so with ``enforce_full_coverage``
            they still count as uncovered.
    ...
    reports = tuple(
        load_matching_weights(
            ...
            skip_mismatched_shapes=skip_mismatched_shapes,
            logger=logger,
        )
